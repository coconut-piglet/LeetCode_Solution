...
双指针，一个src指向输入的字符串的待读取位置，另一个dst指向输出的字符串的待写入位置。一个辅助函数用于记录当前是第一个字段，当前字段的数字和是多少。当前数字为0时，对应地址中某一字段为0的情况，这种情况只存在单一0一种情况，其它则可能独立成段或者与下一个数字组合成段吗，分别对应两次调用。跳出条件为剩余字段数为0，其中src刚好到尾的情况ip会被保留，其余丢弃，和剩余字段数不为0但是src已经超过尾部，直接丢弃。目前的算法存在越界的可能，从输出来看有时候会多写一个分割点。
