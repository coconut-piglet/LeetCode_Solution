...
**\#25 Reverse Nodes in k-Group** `94.76%` `9.68%`

每组的顺序修改策略是获取原始排序的头和尾，从头开始插入到尾的后面，然后更新头，直到头等于尾，然后让前一组尾的next变成新的头，同样需要在链表最前面补一个节点，空间复杂度优化后只上升了1%，可能是因为为了方便补的那个节点占空间较大吧，不补它会需要特殊处理第一组增加代码复杂度想想算了。
...
**\#43 Multiply Strings** `97.12%` `100.00%`

注意字符类是有符号的正数最大127，做法和手算乘法类似但是不会在乘法过程中进位，只是单纯地往对应位增加新的乘积，最后从后向前统一处理进位问题，因为前面提到地字符类的范围限制所以需要一个整形数组存放中间结果，处理完进位问题后再将结果转换为字符串。
...
**\#59 Spiral Matrix II** `100.00%` `18.56%`

同第一代题目，只是把读取的过程修改为写入的过程，另外学习了vector的另外一种初始化方式。

**\#61 Rotate List** `88.97%` `7.14%`

因为是单项列表向左转比向右转方便，因此在遍历完一次获取到尾节点和长度后可以计算出k等效的左移位数，注意这题很贱会有空输入的情况。

**\#62 Unique Paths** `100.00%` `100.00%`

仔细想想初中教的这种题用到的原来是动态规划的思想。

**\#63 Unique Paths II** `82.33%` `28.38%`

在原版的基础上将障碍位的可达线路数设为0即可。
...
**\#71 Simplify Path** `98.72%` `100.00%`

最开始想的是用栈，空白和当前目录符号无视，上一级目录符号弹栈，其它入栈，但生成最终路径的时候从后向前并不是很舒服于是换了链表，因为本题设计的操作只有在尾部增删和遍历，使用链表节约了空间，时间和栈差不多，不使用substr而是在非分隔符时append目录名似乎有加速效果。
...
**\#74 Search a 2D Matrix** `20.49%` `6.06%`

一开始用了两轮二分查找，第一次找在哪行，第二次找在哪列，慢得起飞。优化后将矩阵看成一个排序列表，只做一次二分查找，速度快了一倍。
...
**\#80 Remove Duplicates from Sorted Array II** `95.77%` `5.26%`

快慢指针，一个指向被填入的位置，一个往前找应该填入的数字，一开始用了简单粗暴的方式记录最多2次出现，效率不是很高，后来注意到这是一个排序过的数组，某两个位置的数字相等意味着这两个位置之间的所有数字都相等，因此判断是否出现超过2次只要比较src和dst向前2位是否相等即可，对于长度不大于2的输入可以直接输出。

**\#81 Search in Rotated Sorted Array II** `53.51%` `8.33%`

比起第一代题目，新增可重复的条件会使得出现左右区间的判断变得更加复杂，会出现新的情况比如target大于mid和high且mid等于high等情况。因此在比较完target和mid的关系后，一侧的三个数据还要再相互比较区分四种情况：某一侧升序且target在其中，某一侧升序且target不在其中，某一侧不升序且target在其中和出现mid和和low或high相等的情况。

**\#82 Remove Duplicates from Sorted List II** `90.83%` `7.32%`

快慢指针，三个指针head、fst、snd分别对应当前保留的最后一个头部和当前判断中的节点和下一个待判断的节点，snd与fst的值应不一致。循环条件为snd不为空，循环内容判断三种情况：fst与snd值一致，跳过当前snd；不一致时若snd是fst的next说明fst只出现一次，加到head后面，更新三个指针的位置；否则fst出现超过一次跳过fst，更新三个指针的位置。几种特殊情况，输入为空在最开始解决；最后一个节点无法在循环体内处理，跳出循环后判断fst后一位是否为空，空保留，非空跳过；全部节点都一样，跳出循环后fst仍是最开始的位置并且fst的next非空。
...
**\#86 Partition List** `92.63%` `8.33%`

所有不小于给定x的节点都移到最后面就好了。
...
**\#93 Restore IP Addresses** `100.00%` `100.00%`

双指针，一个src指向输入的字符串的待读取位置，另一个dst指向输出的字符串的待写入位置。输出字符串的长度一定是输入字符串的长度加3。一个辅助函数用于记录当前是第一个字段，当前字段的数字和是多少。当前数字为0时，对应地址中某一字段为0的情况，这种情况只存在单一0一种情况，其它则可能独立成段或者与下一个数字组合成段吗，分别对应两次调用。跳出条件在更新2中修改为任意时刻src超过len；在最后一个数字中开头为0但后续仍有数字；在最后一个数字中开头不为0但后续数字之和超过255；最后一个数字不遇到前面任和一种情况。先前的算法存在越界的可能，在更新1中修正了越界，简化了逻辑，同时意识到了新的写法。更新2中修改了架构，合并了最后一个数字和跳出条件，消除了越界的可能性，增强了性能。
