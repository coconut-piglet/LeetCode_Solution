...
**\#81 Search in Rotated Sorted Array II** `21.77%` `8.33%`

比起第一代题目，新增可重复的条件会使得出现左右区间的判断变得更加复杂，会出现新的情况比如target大于mid和high且mid等于high等情况。因此在比较完target和mid的关系后，一侧的三个数据还要再相互比较区分四种情况：某一侧升序且target在其中，某一侧升序且target不在其中，某一侧不升序且target在其中和出现mid和和low或high相等的情况。待优化。
