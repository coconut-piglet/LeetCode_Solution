...
**\#71 Simplify Path** `42.59%` `100.00%`

最开始想的是用栈，空白和当前目录符号无视，上一级目录符号弹栈，其它入栈，但生成最终路径的时候从后向前并不是很舒服于是换了链表，因为本题设计的操作只有在尾部增删和遍历，使用链表节约了空间，时间和栈差不多，目前慢的原因怀疑是用了substr，可能还是需要自己造轮子。
